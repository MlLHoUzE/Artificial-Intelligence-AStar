...
public:
	...
	bool LoadPlyFileIntoGLBuffer( unsigned int programID, std::string plyFileNameNoBasePath,  bool bEnvironment, bool bPlayer,
	...
	void GetEnvironmentalTriangles(std::vector<sTriangle>& environTris);
	void GetPlayerTriangles(std::vector<sTriangle>& playerTris);

	void getTriangleNormal(std::vector<sTriangle> &vecTris, int triIndex, glm::vec3 &normal);

private: 

	std::vector<sTriangle> m_vecEnvironmentTris;
	std::vector<sTriangle> m_vecPlayerTris;

	bool m_GenerateTriangleInfoFromIndexedVectors(std::string fileName, std::vector<cPlyVertexXYZ_N_STU> &vecPlyVerts, std::vector<cPlyTriFace> &vecPlyIndices);
	bool m_GenerateTriangleInfoFromIndexedVectorsPlayer(std::string fileName, std::vector<cPlyVertexXYZ_N_STU> &vecPlyVerts, std::vector<cPlyTriFace> &vecPlyIndices);
