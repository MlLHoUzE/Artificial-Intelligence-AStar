...
bool cMeshTypeManager::LoadPlyFileIntoGLBuffer( unsigned int programID, std::string plyFile,  bool bEnvironment, bool bPlayer,
                                                bool bMakeFabulousRainbows /*= false*/ )
{
	return this->LoadPlyFileIntoGLBuffer( programID, plyFile, bEnvironment, bPlayer,
...
bool cMeshTypeManager::LoadPlyFileIntoGLBuffer( unsigned int programID, std::string plyFileNameNoBasePath, bool bEnvironment, bool bPlayer,
                                                float vertRed, float vertGreen, float vertBlue,
                                                bool bMakeFabulousRainbows /*=false*/ )
{
	// ********************************************************************
...
//	int offsetInBytesToNormal = offsetof( vert_XYZ_RGB_Nxyz, nX );
	...
	if (bEnvironment)
	{
		this->m_GenerateTriangleInfoFromIndexedVectors(plyFileNameNoBasePath, vecPlyVerts, vecPlyIndices);
	}
	else if (bPlayer)
	{
		this->m_GenerateTriangleInfoFromIndexedVectorsPlayer(plyFileNameNoBasePath, vecPlyVerts, vecPlyIndices);
	}

	return true;
}

bool cMeshTypeManager::m_GenerateTriangleInfoFromIndexedVectors(std::string fileName, std::vector<cPlyVertexXYZ_N_STU> &vecPlyVerts, std::vector<cPlyTriFace> &vecPlyIndices)
{
	...
	{
		this->m_vecEnvironmentTris.push_back(vecTriangles[i]);
	}
...
bool cMeshTypeManager::m_GenerateTriangleInfoFromIndexedVectorsPlayer(std::string fileName, std::vector<cPlyVertexXYZ_N_STU> &vecPlyVerts, std::vector<cPlyTriFace> &vecPlyIndices)
{
	//create a vector of triangles for us to load
	std::vector<sTriangle> vecTriangles;
	...
	for (int i = 0; i != vecTriangles.size(); i++)
	{
		this->m_vecPlayerTris.push_back(vecTriangles[i]);
	}
	//this->m_mapMeshNameToTriInfo[fileName] = vecTriangles;
...
void cMeshTypeManager::GetEnvironmentalTriangles(std::vector<sTriangle>& environTris)
{
	environTris = this->m_vecEnvironmentTris;
	
}

void cMeshTypeManager::GetPlayerTriangles(std::vector<sTriangle>& playerTris)
{
	playerTris = this->m_vecPlayerTris;
}

void cMeshTypeManager::getTriangleNormal(std::vector<sTriangle> &vecTris, int triIndex, glm::vec3 &normal)
{
